...
// Soft pastel tints for emoji avatars (not full gradients)
...
function WADot() {
  return <span className="inline-block h-2 w-2 rounded-full bg-[#25d366]" />;
}

type Group = ReturnType<typeof useGroups>["groups"][number];

// ── Group row ──────────────────────────────────────────────────────────────────
function GroupRow({ group, waLinked }: { group: Group; waLinked: boolean }) {
  const [bg, accent] = tintFor(group.id);

  return (
    <div className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
      {/* Top: emoji + info */}
      <Link
        href={`/groups/${group.id}`}
        className="flex items-center gap-3.5 px-4 pt-4 pb-3"
      >
        {/* Emoji avatar */}
        <div
    ...
    <div className="min-h-[100dvh] bg-[#f5f5f7] pb-28">
      ...
      {!session ? (
        ...
        <div className="px-4 pt-3">
          {/* ── WhatsApp status (one clean line) ────────────────── */}
            ...
            <div className="mb-4">
              {waStatus === "connected" ? (
                ...
                >
                  ...
                  <svg
                    viewBox="0 0 24 24"
                    className="h-4 w-4 shrink-0 fill-white/60"
                  >
          ...
          {loading ? (
            ...
            <div className="rounded-2xl bg-white px-6 py-14 text-center shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
              ...
              <p className="mt-1.5 text-[13px] text-gray-400">
                Create a group to start polling and chatting.
              </p>
              <Link
                href="/groups/new"
            ...
            <div className="space-y-3">
              {groups.map((g) => (
                <GroupRow
                  key={g.id}
                  group={g}
                  waLinked={waLinkedJids.has(g.id)}
                />
              ))}
